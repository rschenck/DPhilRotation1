...
### Parse options and load libraries ###
option_list = list(
  make_option(c("-a", "--act_table"), type="ActivityTable.txt", default=NULL, 
              help="Activity table generated by PreProcessBedFileFeatures.py", metavar="character"),
  make_option(c("-g", "--chromFile"), type="chromFile", default=NULL, help="File containing chromosomes and lengths.", metavar="character")
);

opt_parser = OptionParser(option_list=option_list);
opt = parse_args(opt_parser);

if (is.null(opt$act_table)){
  print_help(opt_parser)
  stop("At least an activity table must be supplied (input activity table).txt", call.=FALSE)
} else {
  library(circlize)
