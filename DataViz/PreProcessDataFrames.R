...
# datamelt <- readRDS("~/Desktop/Oxford/Rotation_1/CNN/DataViz/Rotation1App/Data/FractionOfClassifationsByCell.rds")
...
# ROC Curves #
# rocdf <- read.csv("~/Desktop/Oxford/Rotation_1/CNN/Model/TestRun.29Jan2018.1100.batch128.adam.2018-02-15.22.04/ModelEvalOutput/Batch128adam.roc_curve_data.appended.csv", sep=',', header = T)
rocdf <- read.csv("~/Desktop/Oxford/Rotation_1/CNN/Model/TestRun.Bassett.adam.2018-02-17.18.27/ModelEvalOutput/BassetAdam.roc_curve_data.appended.csv", sep=',', header = T)
# rocdf <- read.csv("~/Desktop/Oxford/Rotation_1/CNN/Model/TestRun.lr0.01.batch128.adam.4layer.dropoutg.2018-02-16.15.02/ModelEvalOutput/4LayerDropG.roc_curve_data.appended.csv", sep=',', header = T)
rocdf$Cell <- as.factor(rocdf$Cell)
rocdf$Karyotype <- as.factor(rocdf$Karyotype)

# rocSum <- read.csv('~/Desktop/Oxford/Rotation_1/CNN/Model/TestRun.29Jan2018.1100.batch128.adam.2018-02-15.22.04/ModelEvalOutput/Batch128adam.roc_curve_data.micromacro.csv', sep=',', header=T)
rocSum <- read.csv('~/Desktop/Oxford/Rotation_1/CNN/Model/TestRun.Bassett.adam.2018-02-17.18.27/ModelEvalOutput/BassetAdam.roc_curve_data.micromacro.csv', sep=',', header=T)
# rocSum <- read.csv('~/Desktop/Oxford/Rotation_1/CNN/Model/TestRun.lr0.01.batch128.adam.4layer.dropoutg.2018-02-16.15.02/ModelEvalOutput/4LayerDropG.roc_curve_data.micromacro.csv', sep=',', header=T)
rocSum$SumType <- as.character(rocSum$SumType)

rocSumAll <- subset(rocSum, rocSum$SumType=="microall" | rocSum$SumType=="macroall")
macro <- paste("Macro-Average (AUC: ",round(unique(subset(rocSumAll, rocSumAll$SumType=="macroall")$AUC),4),")", sep ="")
micro <- paste("Micro-Average (AUC: ",round(unique(subset(rocSumAll, rocSumAll$SumType=="microall")$AUC),4),")", sep ="")

# Overall Macro Micro
P <- ggplot() + geom_line(data=rocdf, aes(x=fpr, y=tpr, group=Cell), alpha=0.2, size=0.2, color="grey", inherit.aes = F) + 
  geom_line(data=rocSumAll, aes(x=fpr, y=tpr, colour=SumType), size=0.5, inherit.aes = F) +
  theme_minimal() +
  scale_color_manual(labels = c(macro, micro), values = c("darkorange2", "chartreuse3")) + 
  xlab("False Positive Rate (1-Specificity)") + ylab("True Positive Rate (Sensitivity)") +
  ggtitle("BassetAdam") + geom_abline(slope=1, intercept=0, size=0.2, linetype="dashed") +
  theme(legend.position = c(0.8, 0.25)) + guides(color=guide_legend(title=""))
P
# ggsave(filename="~/Desktop/Oxford/Rotation_1/ResultsAndFigures/BassetAdam.roc.png", plot=P, units = "in", device='png', width=10, height=10)

# Karyotype Macro Micro
rocSumAll <- subset(rocSum, rocSum$SumType !="microall" & rocSum$SumType!="macroall")
macro_cancer <- paste("Macro-Average Cancer (AUC: ",round(unique(subset(rocSumAll, rocSumAll$SumType=="macro_cancer")$AUC),4),")", sep ="")
macro_normal <- paste("Macro-Average Normal (AUC: ",round(unique(subset(rocSumAll, rocSumAll$SumType=="macro_normal")$AUC),4),")", sep ="")
macro_unspecified <- paste("Macro-Average Unspecified (AUC: ",round(unique(subset(rocSumAll, rocSumAll$SumType=="macro_unspecified")$AUC),4),")", sep ="")
micro_cancer <- paste("Micro-Average Cancer (AUC: ",round(unique(subset(rocSumAll, rocSumAll$SumType=="micro_cancer")$AUC),4),")", sep ="")
micro_normal <- paste("Micro-Average Normal (AUC: ",round(unique(subset(rocSumAll, rocSumAll$SumType=="micro_normal")$AUC),4),")", sep ="")
micro_unspecified <- paste("Micro-Average Normal (AUC: ",round(unique(subset(rocSumAll, rocSumAll$SumType=="micro_unspecified")$AUC),4),")", sep ="")

P <- ggplot() + geom_line(data=rocdf, aes(x=fpr, y=tpr, group=Cell), alpha=0.2, size=0.2, color="grey", inherit.aes = F) +
  geom_line(data=rocSumAll, aes(x=fpr, y=tpr, colour=SumType), size=0.5, inherit.aes = F) +
  theme_minimal() +
  scale_color_manual(labels = c(macro_cancer, macro_normal,macro_unspecified,micro_cancer,micro_normal,micro_unspecified), values = c("darkorange2", "chartreuse3", "darkolivegreen3","darkgoldenrod2","darkslategray4","dodgerblue2")) +
  xlab("False Positive Rate (1-Specificity)") + ylab("True Positive Rate (Sensitivity)") +
  ggtitle("BassetAdam") + geom_abline(slope=1, intercept=0, size=0.2, linetype="dashed") +
  theme(legend.position = c(0.7, 0.25)) + guides(color=guide_legend(title=""))
P
# ggsave(filename="~/Desktop/Oxford/Rotation_1/ResultsAndFigures/BassetAdam.karyotypeROC.png", plot=P, units = "in", device='png', width=10, height=10)

#--- Fraction of Accessibility TRUE and Predicted ---#
# fracAcc <- read.csv('~/Desktop/Oxford/Rotation_1/CNN/DataViz/Rotation1App/Data/FractionAccessiblePred.txt', sep='\t', header = T, stringsAsFactors = F)
# vizdf <- subset(fracAcc, fracAcc$TrainRun=="Batch128adam")
# vizdf$Diff <- vizdf$TrueFraction - vizdf$PredFraction
# vizdf2 <- melt(vizdf, id.vars = c("TrainRun", "Karyotype", "Cell"))
# 
# p <- ggplot(vizdf2, aes(x=Cell, y=value, group=variable, color=variable)) + geom_bar(stat="identity", position=position_dodge(width=0.5))
# p
# 
# p <- ggplot(vizdf, aes(x=Cell, y=Diff, fill=Karyotype)) + geom_bar(stat="identity") + 
#   scale_y_continuous(limits=c(min(vizdf$Diff)-0.01,max(vizdf$Diff+0.02))) + facet_grid(~Karyotype, scales = "free", space = "free") + 
#   geom_hline(yintercept=0, color="black", linetype="dashed") + theme_minimal() + 
...
#   xlab("Cell Line") + ylab("Difference in Accessibility Fraction")
# p
# 
